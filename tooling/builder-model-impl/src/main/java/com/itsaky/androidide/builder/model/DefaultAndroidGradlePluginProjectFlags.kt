...
class DefaultAndroidGradlePluginProjectFlags(val booleanFlagMap: Map<BooleanFlag, Boolean?>) :
  ...
  companion object {
    ...
    @JvmStatic
    fun fromProjectFlags(flags: AndroidGradlePluginProjectFlags): DefaultAndroidGradlePluginProjectFlags {
      val flagMap = BooleanFlag.values().associateWith { flag ->
        try {
          flags.getFlagValue(flag.name)
        } catch (e: Exception) {
          null
        }
      }
