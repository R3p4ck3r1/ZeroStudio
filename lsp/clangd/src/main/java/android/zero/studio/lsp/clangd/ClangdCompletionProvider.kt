 ...
 *        |
 *        v
 * [ClangdCompletionProvider.complete] (携带行列号及上下文信息)
 *        | (校验取消状态 & 文件后缀验证)
 *        v
 * [ILanguageServerRegistry] -> 获取 "clangd-native" 服务器实例
 *        |
 *        v
 * [ClangdLanguageServer.complete] -> (组装 JSON-RPC Request 经 Stdio 发送至 clangd 进程)
 *        |
 *        v
...
class ClangdCompletionProvider : AbstractServiceProvider(), ICompletionProvider {
    ...
    override fun complete(params: CompletionParams): CompletionResult {
        ...
        // 3. 从全局注册表中寻找注册的 ClangdLanguageServer
        val server = ILanguageServerRegistry.getDefault().getServer("clangd-native") 
            ?: return CompletionResult.EMPTY

        // 4. 委托给 ClangdLanguageServer，内部会转为 JSON-RPC 调用
