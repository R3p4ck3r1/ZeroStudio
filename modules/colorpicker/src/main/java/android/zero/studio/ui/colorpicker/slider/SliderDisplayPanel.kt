 ...
 *   nul [[SliderDisplaySaturationHSV] is added to Column of sliders.
 ...
 *   nul [[SliderDisplayLightnessHSL] is added to Column of sliders.
 * @param onAlphaChange lambda in which [alpha] should be updated. when this lambda is not nul
...
fun SliderDisplayPanelRGBA(
  ...
  Column(modifier) {
    onRedChange?.let { onRedChanged ->
      SliderDisplayRedRGB(modifier = Modifier, red = red, onValueChange = onRedChanged)
    }

    onGreenChange?.let { onGreenChanged ->
      SliderDisplayGreenRGB(modifier = Modifier, green = green, onValueChange = onGreenChanged)
    }

    onBlueChange?.let { onBlueChanged ->
      SliderDisplayBlueRGB(modifier = Modifier, blue = blue, onValueChange = onBlueChanged)
    }
