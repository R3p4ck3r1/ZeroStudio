...
import android.zero.studio.ui.colorpicker.R
import com.smarttoolfactory.extendedcolors.util.ColorUtil
import com.smarttoolfactory.extendedcolors.util.fractionToIntPercent
import kotlin.math.roundToInt

@Composable
fun ColorDisplayWithClipboard(modifier: Modifier = Modifier, colorData: ColorData) {
  ...
  Column(
      modifier =
          modifier
              .shadow(2.dp, if (expanded) RoundedCornerShape(25) else RoundedCornerShape(50))
              .width(200.dp)
...
fun ColorDisplayWithClipboard(modifier: Modifier = Modifier, color: Color) {
  ...
  Column(
    ...
    Row {
      Column { Text(text = hexText, fontSize = 20.sp, color = textColor) }
      Spacer(modifier = Modifier.weight(1f))
      IconButton(
          onClick = {
            Toast.makeText(context, "Copied $hexText", Toast.LENGTH_SHORT).show()
    ...
    AnimatedVisibility(visible = expanded) {
      Column {
        ...
        Text(text = "R: ${rgb[0]} G: ${rgb[1]} B: ${rgb[2]}", fontSize = 12.sp, color = textColor)
        Text(
            text =
