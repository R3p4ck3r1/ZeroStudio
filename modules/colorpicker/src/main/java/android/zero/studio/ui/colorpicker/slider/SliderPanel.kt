...
/**
 * Composable that contains variant number of [ColorfulSlider], and current value from slider
 * depending on **onChange** callbacks are null or not.
 *
 ...
 * @param onSaturationChange lambda in which [saturation] should be updated. when this lambda is not
 *   nul [[SliderSaturationHSV] is added to Column of sliders.
...
fun SliderPanelHSV(
  ...
  Column(modifier) {
    ...
    onValueChange?.let { onValueChanged ->
      SliderValueHSV(modifier = Modifier, hue = hue, value = value, onValueChange = onValueChanged)
    }

    onAlphaChange?.let { onAlphaChanged ->
      SliderAlphaHSV(modifier = Modifier, hue = hue, alpha = alpha, onValueChange = onAlphaChanged)
    }
  }
}
 ...
 * @param onSaturationChange lambda in which [saturation] should be updated. when this lambda is not
 *   nul [[SliderSaturationHSL] is added to Column of sliders.
 * @param onLightnessChange lambda in which [lightness] should be updated. when this lambda is not
 *   nul [[SliderLightnessHSL] is added to Column of sliders.
 * @param onAlphaChange lambda in which [alpha] should be updated. when this lambda is not nul
...
fun SliderPanelHSL(
  ...
  Column(modifier) {
    ...
    onLightnessChange?.let { onValueChanged ->
      SliderLightnessHSL(modifier = Modifier, lightness = lightness, onValueChange = onValueChanged)
    }

    onAlphaChange?.let { onAlphaChanged ->
      SliderAlphaHSL(modifier = Modifier, hue = hue, alpha = alpha, onValueChange = onAlphaChanged)
    }
  }
}

/**
 * Composable that contains variant number of Sliders with Title on left, [ColorfulSlider], and
 * current value from slider depending on **onChange** callbacks are null or not.
 *
 * Based on availability of callbacks order of sliders are placed in order from top to bottom
 *
 * ```
...
fun SliderPanelRGBA(
  ...
  Column(modifier) {
    onRedChange?.let { onRedChanged ->
      SliderRedRGB(modifier = Modifier, red = red, onValueChange = onRedChanged)
    }

    onGreenChange?.let { onGreenChanged ->
      SliderGreenRGB(modifier = Modifier, green = green, onValueChange = onGreenChanged)
    }

    onBlueChange?.let { onBlueChanged ->
      SliderBlueRGB(modifier = Modifier, blue = blue, onValueChange = onBlueChanged)
    }
