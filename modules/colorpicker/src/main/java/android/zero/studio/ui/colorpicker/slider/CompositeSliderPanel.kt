...
/**
 * Slider display panel that allows user to pick color between HSV, HSL or RGB color models using
 * other **CompositeSliderPanel**s.
 *
 * * Input color model determines in which color model this Composable will use to calculate which
 *   depends on current Sliders of preference, if RGB color model is selected then [inputColorModel]
 *   will also be [ColorModel.RGB]. This is for matching sliders with color picked using them.
 *
    ...
    ColorModel.HSL -> {
      ...
      when (compositeColor) {
        ...
        is ColorHSV -> {
          ...
          ColorHSL(hslArray[0], hslArray[1], hslArray[2], alpha)
        }
        is ColorRGB -> {
          ...
          ColorHSL(rgbArray[0], rgbArray[1], rgbArray[2], alpha)
        }
    ...
    ColorModel.HSV -> {
      ...
      when (compositeColor) {
        is ColorHSL -> {
          ...
          val alpha = compositeColor.alpha
          val hsvArray = HSLUtil.hslToHSV(hue = hue, saturation = saturation, lightness = lightness)
          ColorHSV(hsvArray[0], hsvArray[1], hsvArray[2], alpha)
        }
        is ColorHSV -> {
          compositeColor
        }
        is ColorRGB -> {
          ...
          ColorHSV(rgbArray[0], rgbArray[1], rgbArray[2], alpha)
        }
    ...
    ColorModel.RGB -> {
      ...
      when (compositeColor) {
        is ColorHSL -> {
          ...
          val rgbArray = HSLUtil.hslToRGBFloat(hue = hue, saturation = saturation, lightness)
          ColorRGB(rgbArray[0], rgbArray[1], rgbArray[2], alpha)
        }
        is ColorHSV -> {
