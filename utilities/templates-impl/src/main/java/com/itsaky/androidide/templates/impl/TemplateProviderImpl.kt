...
import com.itsaky.androidide.templates.impl.chaquopy.litho.chaquopyLithoDemoProject
import com.itsaky.androidide.templates.impl.chaquopy.xml.chaquopyXmlDemoProject
 ...
 *
 * @property templatesByCategory The core data structure that holds templates grouped by their
 *   [TemplateCategory]. It uses a [ConcurrentHashMap] for thread-safe operations.
 ...
 * 2. `initializeTemplates()` populates the provider with default templates, assigning each to a
 *    specific category (e.g., `TemplateCategory.Mobile`).
 * 3. The public methods (`getRegisteredCategories`, `getTemplatesFor`, etc.) provide access to this
 *    categorized data.
 * 4. `reload()` and `release()` methods manage the lifecycle of the template cache.
 *
 * Usage: To add a new template, simply call `registerTemplate()` within `initializeTemplates()`:
 * ```kotlin
...
class TemplateProviderImpl : ITemplateProvider {
  ...
  /**
   * Initializes and registers all default project templates into their respective categories. This
   * method serves as the central point for template registration. To add new templates, they should
   * be registered here.
   *
   * Work Flow:
   * 1. Clears any previously loaded templates to ensure a fresh state.
   * 2. Calls `registerTemplate` for each default project template, assigning it to the
   *    `TemplateCategory.Mobile` category.
   *
...
// 注册子类别容器
  ...
  /**
   * Registers a template under a specific category. If the category does not yet exist, it is
   * created. This method is thread-safe.
   *
  ...
  override fun registerTemplate(category: TemplateCategory, template: Template<*>) {
    val templates =
        templatesByCategory.computeIfAbsent(category) {
  ...
  /**
   * Retrieves a list of all categories that currently have one or more templates registered. The
   * list is sorted according to the predefined order in [TemplateCategory.defaultCategories].
   *
  ...
  override fun getRegisteredCategories(): List<TemplateCategory> {
    ...
    val defaultOrder =
        TemplateCategory.defaultCategories().withIndex().associate { it.value.key to it.index }
    // Sort the existing keys based on the default order. Categories not in the default list are
    // placed at the end.
   ...
   * @return An immutable list of [Template] instances. Returns an empty list if the category is not
   *   found or has no templates.
   */
  override fun getTemplatesFor(category: TemplateCategory): List<Template<*>> {
    return templatesByCategory[category]?.let { Collections.unmodifiableList(it.toList()) }
        ?: emptyList()
  }
  ...
  /**
   * Releases all resources held by the templates and clears all registered templates and
   * categories. This should be called to free up memory when the provider is no longer needed.
   */
  override fun release() {
    templatesByCategory.values.forEach { list -> list.forEach { it.release() } }
    templatesByCategory.clear()
  }
}
