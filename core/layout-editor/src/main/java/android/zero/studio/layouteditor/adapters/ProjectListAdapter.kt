...
import android.zero.studio.layouteditor.activities.PreviewLayoutActivity
import android.zero.studio.layouteditor.databinding.ListProjectFileBinding
...
class ProjectListAdapter(private val projects: MutableList<ProjectFile>) :
  ...
  class ViewHolder(var binding: ListProjectFileBinding) : RecyclerView.ViewHolder(binding.root) {
  ...
  override fun onBindViewHolder(holder: ViewHolder, position: Int) {
    ...
    holder.itemView.animation =
        AnimationUtils.loadAnimation(holder.itemView.context, R.anim.project_list_animation)
  ...
  private fun renameProject(v: View, position: Int) {
    ...
    builder.setPositiveButton(string.rename) { _, _ ->
      ...
      val newPath = "${path.substring(0, path.lastIndexOf("/"))}/${editText.text.toString()}"
  ...
  private fun openProject(v: View, position: Int) {
    val intent = Intent(v.context, LayoutsEditorActivity::class.java)
  ...
  private fun previewLayout(v: View, position: Int) {
    ...
    MaterialAlertDialogBuilder(v.context)
        ...
        .setAdapter(ArrayAdapter(v.context, android.R.layout.simple_list_item_1, layouts)) { d, w ->
