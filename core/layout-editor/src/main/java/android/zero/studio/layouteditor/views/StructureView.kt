package android.zero.studio.layouteditor.views
...
import android.zero.studio.layouteditor.databinding.LayoutStructureViewItemBinding
import android.zero.studio.layouteditor.managers.IdManager.idMap
...
class StructureView @JvmOverloads constructor(context: Context, attrs: AttributeSet? = null) :
  ...
  private fun peek(view: View, depth: Int) {
    var nextDepth = depth
    val binding = LayoutStructureViewItemBinding.inflate(inflater, null, false)
    ...
    if (view.id == -1 || idMap[view] == null) {
      ...
      TooltipCompat.setTooltipText(binding.mainView, view.javaClass.superclass.simpleName)
    } else {
    ...
    if (v is ViewGroup) {
      for (i in 0 until v.childCount) {
        ...
        if (child.id == R.id.view_name)
            textViewMap[child as TextView]?.let { onItemClickListener(it) }
      }
    }
  }
