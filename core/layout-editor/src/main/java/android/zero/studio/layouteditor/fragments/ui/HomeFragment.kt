package android.zero.studio.layouteditor.fragments.ui
...
import android.zero.studio.layouteditor.adapters.ProjectListAdapter
import android.zero.studio.layouteditor.databinding.FragmentLayoutHomeBinding
...
class HomeFragment : Fragment() {
  private var binding: FragmentLayoutHomeBinding? = null
  ...
  override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
    ...
    adapter.onDeleteCallback = { updateNoProjectsViewVisibility() }
  ...
  private fun showCreateProjectDialog() {
    ...
    val bind = TextinputlayoutBinding.inflate(layoutInflater)
    ...
    builder.setPositiveButton(string.create) { _, _ ->
      createProject(bind.textinputEdittext.text.toString())
    }
  ...
  private fun createProject(name: String) {
    ...
    // ((LayoutFile)
    // getActivity().getIntent().getExtras().getParcelable(Constants.EXTRA_KEY_LAYOUT)).saveLayout("");
    ...
    val intent = Intent(requireContext(), LayoutsEditorActivity::class.java)
