...
abstract class FilePicker(private val actvty: AppCompatActivity) {
  ...
  init {
    ...
    this.getFile =
        actvty.registerForActivityResult(ActivityResultContracts.GetContent()) { onPickFile(it) }
    ...
    this.reqPermission =
        actvty.registerForActivityResult(ActivityResultContracts.RequestPermission()) {
  ...
  private fun onRequestPermission(isGranted: Boolean) {
    if (isGranted)
        make(actvty.findViewById(android.R.id.content), R.string.permission_granted)
            .setSlideAnimation()
            .showAsSuccess()
    else
        make(actvty.findViewById(android.R.id.content), R.string.permission_denied)
            .setSlideAnimation()
            .showAsError()
  }
    ...
    if (isImageType) {
      if (LayoutEditorContext.isAtLeastTiramisu) {
        if (
          ...
          actvty.checkSelfPermission(Manifest.permission.READ_EXTERNAL_STORAGE) ==
              PackageManager.PERMISSION_DENIED
      ) {
